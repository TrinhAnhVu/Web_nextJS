...
const OrderDetail = ({ orderDetail, state, dispatch}) => {
    ...
    const handleDelivered = (order) =>{
        ...
        .then(res =>{
            ...
            const {paid, dataOfPayment, method, delivered} = res.result 
            dispatch(updateItem(orders, order._id, {
                ...order, paid, dataOfPayment, method, delivered
            }, 'ADD_ORDERS'))
        ...
        <>
            {
                orderDetail.map(order => (
                    <div key={order._id} style={{ margin: '20px auto' }} className="row justify-content-around">
                                ...
                                <div>
                                    <h4>Order Items</h4>
                                    {
                                        order.cart.map(item => (
                                            ...
                                            align-items-center" key={item._id} style={{ maxWidth: '500px' }}>
                        ...
                        {
                            !order.paid && auth.user.role !== 'admin' &&                            <div className="p-4">
